    ...
    <div className="w-full max-w-[340px] bg-white/[0.01] border border-white/10 rounded-xl p-4 flex flex-col gap-3.5 shadow-2xl backdrop-blur-md relative overflow-hidden select-none">
      ...
      <div className="grid grid-cols-[1.2fr_auto_1fr] items-center gap-2">
        <div className="flex flex-col gap-2 z-10">
          {[
            { name: "BOQ_Master.xlsx", type: "sheet" },
            { name: "Layout_L2.dwg", type: "cad" },
            { name: "PPA_Execution.pdf", type: "pdf" }
          ].map((file, i) => (
  ...
  return (
    <div className="w-full max-w-[340px] bg-white/[0.01] border border-white/10 rounded-xl p-4 flex flex-col gap-3 shadow-2xl backdrop-blur-md relative overflow-hidden select-none">
      ...
      <div className="flex flex-col gap-2 relative pl-3.5 border-l border-white/10 text-left">
        {[
          { title: "Scope Baselined", diff: "+ 1,280 WBS Elements", status: "Verified", time: "10m" },
          { title: "Invoice Matching", diff: "+ $140,230 Verified", status: "Match", time: "1h" },
          { title: "Change Order Signed", diff: "+ $14,000 / - $2,100", status: "Signed", time: "3h" }
        ].map((item, i) => (
  ...
  return (
    <div className="w-full max-w-[280px] h-[160px] relative flex items-center justify-center select-none">
      ...
      <div className="absolute top-[10px] left-[15px] bg-white/[0.01] border border-white/10 rounded-lg p-1 px-2 flex items-center gap-1 shadow-md">
        ...
        <span className="text-[0.55rem] text-slate-300 font-mono">HQ Control</span>
      </div>
      <div className="absolute top-[10px] right-[10px] bg-white/[0.01] border border-white/10 rounded-lg p-1 px-2 flex items-center gap-1 shadow-md">
        ...
        <span className="text-[0.55rem] text-slate-300 font-mono">Field App</span>
      </div>
      <div className="absolute bottom-[8px] left-[85px] bg-white/[0.01] border border-white/10 rounded-lg p-1 px-2 flex items-center gap-1 shadow-md">
        ...
        <span className="text-[0.55rem] text-slate-300 font-mono">Owner Portal</span>
      </div>
    </div>
  );
}
...
function ChangeOrdersWidget() {
  return (
    <div className="w-full max-w-[280px] bg-white/[0.01] border border-white/10 rounded-xl p-3 flex flex-col gap-2.5 shadow-2xl backdrop-blur-md relative overflow-hidden select-none">
      <div className="flex items-center justify-between">
        <span className="text-[0.55rem] text-slate-400 font-mono uppercase tracking-wider">Baseline vs Impact</span>
        <span className="text-[0.5rem] text-red-400 flex items-center gap-0.5 animate-pulse">
          <span className="w-1.5 h-1.5 rounded-full bg-red-400" /> Conflict Alert
        </span>
      </div>
      <div className="flex flex-col gap-2 text-left">
        {[
          { name: "Pile Foundations", w: "45%", color: "bg-accent/80", col: 1 },
          { name: "Module Mounting", w: "30%", color: "bg-red-500/50 border border-red-500/40", col: 2, conflict: true },
          { name: "Cable Trenching", w: "35%", color: "bg-accent/20 border border-accent/15", col: 3 }
        ].map((item, i) => (
          <div key={i} className="flex flex-col gap-0.5">
            ...
            <div className="grid grid-cols-5 h-2.5 bg-white/[0.02] border border-white/5 rounded overflow-hidden relative">
              <div 
                className={`h-full rounded-r ${item.color} ${item.conflict ? 'animate-pulse' : ''}`}
                style={{ 
  ...
  return (
    <div className="w-full max-w-[280px] h-[160px] relative flex items-center justify-center select-none overflow-hidden">
      ...
      <div className="absolute top-[32px] left-[55px] flex items-center gap-1 z-10">
        ...
        <span className="bg-red-500/10 border border-red-500/30 text-red-400 text-[0.45rem] font-bold py-0.2 px-0.8 rounded font-mono uppercase tracking-wider shadow-sm">Weather Risk</span>
      </div>
      <div className="absolute bottom-[40px] right-[30px] flex items-center gap-1 z-10">
        ...
        <span className="bg-accent/10 border border-[#00bf99]/30 text-accent text-[0.45rem] font-bold py-0.2 px-0.8 rounded font-mono uppercase tracking-wider shadow-sm">Delay Alert</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div
      className={`bg-[#080a0f] border border-white/5 rounded-2xl p-6 sm:p-8 relative overflow-hidden flex flex-col justify-between transition-colors duration-300 hover:bg-[#0c0e15] hover:border-white/10 ${
        isWide ? "min-h-[320px] md:min-h-[380px]" : "min-h-[380px] md:min-h-[440px]"
      } ${className}`}
    >
      ...
      {isWide ? (
        ...
        <div className="flex flex-col justify-between h-full relative z-[2] gap-6 text-left">
          <div className="flex flex-col gap-2">
            <span className="text-[0.75rem] font-primary font-semibold text-accent uppercase tracking-widest">{feature.subtitle}</span>
            <h3 className="font-accent text-xl text-secondary-light font-bold leading-tight">{feature.title}</h3>
            <p className="font-primary text-[0.88rem] text-secondary-mid leading-relaxed opacity-80 mt-2">{feature.description}</p>
          </div>
          <div className="w-full flex justify-center items-center overflow-hidden h-[160px] mt-auto">
            {widgets[index]}
          </div>
        </div>
      )}
        ...
        <div className="max-w-[1400px] mx-auto w-full mb-20 px-4 flex flex-col items-center relative z-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16 w-full">
            {[
              ...
              >
                <div className="relative z-[2] flex flex-col gap-6 h-full text-left">
                  <div className="w-12 h-12 bg-white/5 border border-white/10 rounded-xl flex items-center justify-center text-white transition-all duration-300 hover:scale-110">
                    {pillar.icon}
                  </div>

                  <div className="flex flex-col gap-2">
                    <h4 className="font-accent text-xl text-secondary-light font-semibold">{pillar.title}</h4>
                    <p className="font-primary text-[0.9rem] text-secondary-mid leading-relaxed opacity-80 group-hover:opacity-100 transition-opacity duration-300">{pillar.desc}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          >
            <button
              ...
              className="group cta-btn bg-accent text-primary-bg border-none py-4 px-10 rounded-lg font-bold cursor-pointer font-primary text-sm tracking-widest uppercase shadow-[0_4px_20px_rgba(0,191,153,0.25)] transition-[background-color,transform,box-shadow] duration-200 hover:bg-accent-hover hover:scale-105 focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent active:scale-95 disabled:opacity-50 inline-flex items-center gap-2"
            >
